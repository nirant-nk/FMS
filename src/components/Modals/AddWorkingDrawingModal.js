...
import useAddWorkingDrawing from '../../hooks/useAddWorkingDrawing';

const AddWorkingDrawingModal = ({visible, onRequestClose, Title, getData}) => {
  const {
    formData,
    formDataError,
    handleInputChange,
    ...
    handleNextFocus,
    LoadingVisible,
  } = useAddWorkingDrawing(onRequestClose, getData);
        ...
        <TouchableWithoutFeedback onPress={onRequestClose}>
          <View ref={ViewRef} style={styles.modalContainer}>
            <Pressable style={styles.MainContainer}>
              ...
              <TextIconButton
                onPress={() => onDocumentSelect('WorkingDrawing')}
                TextData={'Upload'}
                Icon={
                  <Icons.Feather
                    name={'upload'}
              ...
              />
              {formData?.WorkingDrawing?.name ? (
                <CustomeText_10pt
                  TextData={formData?.WorkingDrawing?.name}
                  style={{marginTop: responsiveHeightPixel(2)}}
                />
              ) : null}
              <ErrorMSG Error={formDataError.WorkingDrawing} marginTop={5} />
...
export default AddWorkingDrawingModal;
