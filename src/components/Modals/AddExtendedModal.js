...
import useAddExtendedContract from '../../hooks/useAddExtendedContract';

const AddExtendedModal = ({visible, onRequestClose, getData, isUpdate}) => {
  const {
    formData,
    formDataError,
    handleInputChange,
    handleSubmit,
    LoadingVisible,
  } = useAddExtendedContract(onRequestClose, getData);
        ...
        <TouchableWithoutFeedback onPress={onRequestClose}>
          <View ref={ViewRef} style={styles.modalContainer}>
            <Pressable style={styles.MainContainer}>
              <CustomeText_18pt
                TextData={`${isUpdate ? 'Update' : 'Add'} Extended Contract`}
...
export default AddExtendedModal;
