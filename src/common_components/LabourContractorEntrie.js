...
const LabourContractorEntrie = ({item}) => {
  return (
    <View style={styles_1.MainStyle}>
      ...
      <View
        ...
        <View style={{flex: 1, rowGap: responsiveHeightPixel(5)}}>
          <CustomeText_14pt
            TextData={`Name of worker : ${item?.labour_contractor?.worker_name}`}
          />
          <CustomeText_14pt
            TextData={`Helper Qty : ${item?.labour_contractor?.helper_quantity}`}
          />
          <CustomeText_14pt
            TextData={`Karagir Qty : ${item?.labour_contractor?.karagir_quantity}`}
          />
...
export default LabourContractorEntrie;
